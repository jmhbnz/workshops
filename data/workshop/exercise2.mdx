...
Before we begin, if you would like a bit more background on what a container is or why they are important click the following link to learn more: [https://www.redhat.com/en/topics/containers#overview](https://www.redhat.com/en/topics/containers#overview)
...
Click the **Quick create** button in the top right menu and select **Container images**.

In the **Image name from external registry** field enter the following:
...
OpenShift will create a [`Deployment`](https://docs.openshift.com/container-platform/4.20/applications/deployments/what-deployments-are.html) that will pull this container image if it does not exist already on the cluster and create a [`Pod`](https://docs.openshift.com/container-platform/4.20/nodes/pods/nodes-pods-using.html) that our container will run inside. You will be taken back to the **Topology** view in the web console which will show the new "Parksmap" application.
...
> Note: A pod is the smallest deployable unit in Kubernetes and is effectively a grouping of one or more individual containers. Any containers deployed within a pod are guaranteed to run on the same machine. It is very common for pods in kubernetes to only hold a single container, although sometimes auxiliary services can be included as additional containers in a pod when we want them to run directly alongside our application container.

## 2.3 - Accessing the application

Now that we have the ParksMap application deployed. How do we access it??

This is where OpenShift [`Routes`](https://docs.openshift.com/container-platform/4.20/networking/routes/route-configuration.html) and [`Services`](https://docs.openshift.com/container-platform/4.20/rest_api/network_apis/service-v1.html) come in.
...
In the web console **Workload** perspective, open the **Topology** view.
...
## 2.5 - Checking application resource usage
...
In the web console **Observe** perspective, open the **Dashboards** view.
...
Feel free to explore these concepts further and when you're ready you can move on to [Exercise 3](/workshop/exercise3).
